...
<head>
    ...
    <title>Edit Artikel — HealthyLife Admin</title>
    ...
    <style>
        ...
        .img-preview{width:200px;height:140px;object-fit:cover;border-radius:16px;border:3px solid #e5e7eb;}
...
<main class="main">
    ...
    <div class="flex justify-between items-center bg-white rounded-2xl px-8 py-5 mb-8 shadow-sm">
        <div>
            <h1 class="text-2xl font-extrabold text-moss">✏️ Edit Artikel</h1>
            <p class="text-herb text-sm mt-1">{{ $article->title }}</p>
        </div>
            ...
            <div class="mb-6">
                ...
                @if($article->image)
                    <div class="mb-3 flex items-center gap-4">
                        <img src="{{ $article->image_url }}" alt="{{ $article->title }}" class="img-preview">
                        <div>
                            <p class="text-moss text-xs font-semibold">Gambar saat ini</p>
                            <p class="text-herb text-xs opacity-60">{{ $article->image }}</p>
                        </div>
                    </div>
                @endif
                ...
                <p class="text-herb text-xs mt-2 opacity-60">Kosongkan jika tidak ingin mengubah gambar. Format: JPG, PNG, WebP — Maks 2MB</p>
                <img id="imagePreview" class="img-preview mt-3" style="display:none;" alt="Preview baru">
            </div>
